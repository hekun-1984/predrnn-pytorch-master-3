...
class InputHandle:
    def __init__(self, input_param):#mnist数据集的构造器，配置参数
    ...
    def begin(self, do_shuffle = True): ######################开始？？？
        self.indices = np.arange(self.total(),dtype="int32") #indices：指数
        if do_shuffle:
            random.shuffle(self.indices) #shuffle：洗牌，打乱
        self.current_position = 0 #
